...
from fairseq.data.data_utils import compute_mask_indices, compute_mask_indices_ema_loss
    ...
    def contextualized_features(
        ...
        if mask:
            if clone_batch > 1:
                ...
                mask_pred_loss = mask_pred_loss.repeat_interleave(clone_batch, 0)
        ...
        if precomputed_mask is not None:
            ...
            if mask_prob > 0:
                if cfg.mask_length == 1:
                    ...
                    if mask_pred_loss is not None:
                        mask = compute_mask_indices_ema_loss(
                            (B, T),
                            padding_mask,
                            mask_pred_loss,
                            mask_prob,
