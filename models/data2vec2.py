...
class Data2VecMultiModel(BaseFairseqModel):
    ...
    def forward(
        ...
        if features_only:
            ...
            #print('precomputed_mask: ', precomputed_mask.shape)
            #First we will get the output from the teacher model. This ensures that we are getting initial loss values from
            #loss prediction decoders to generate masks
            loss_pred, y = self.forward_teacher(feature_extractor, 
                                mode, 
                                target, 
                                source, 
                                padding_mask, 
            ...
            #Next we will get the features from the student model. Only the unmasked information is passed to the student
            extractor_out = feature_extractor(
                source,
                padding_mask,
                mask,
        ...
        # In current setup we have three decoders (One is the mask token prediction decoder (feature_extractor.decoder),
        # and other 2 are the loss prediction decoders (feature_extractor.decoder_pred,) attached across student and teacher model)
        if self.shared_decoder is not None:
        ...
        # encoder_mask combines the mask generating from prediction loss and randomly (We are using current
        # training epoch as a parameter to calculate keep_ratio
        ...
        if self.cfg.d2v_loss > 0:
            ...
            for i, x in enumerate(xs):
                ...
                #We are calculating a relative loss to compare the loss_pred with target_loss on only masked tokens
                if self.cfg.learning_loss > 0:
                    learning_loss = self.forward_learning_loss(x_loss_pred, 
                                                               encoder_mask.mask, 
                                                               target_reg_loss, 
                                                               True)
    ...
    def forward_learning_loss(self, loss_pred, mask, loss_target, relative=True):
        ...
        loss_target: [N, L]
        """
        masked_num = int(mask[0].sum().detach().cpu().tolist())
        loss_pred = loss_pred.reshape(mask.shape[0], masked_num)
        loss_target = loss_target.reshape(mask.shape[0], masked_num)
        if relative:
            ...
            #precision control, converting float16 to float32
            loss_pos = loss_pos.float()
            loss_neg = loss_neg.float()
